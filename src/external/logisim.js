...
class LogiSim {
    ...
    static async import(app, fileHandle, text) {
        ...
        if (instance.#problems.size > 0 || instance.#missing.size > 0) {
            const circuit = new Circuit(app, "# Import issues");
            let y = 0;
    ...
    async #importFile(lid, project, whoNeedsIt) {
        ...
        for (const lib of project.lib) {
            ...
            if (type === 'file') {
                ...
                if (fileProject) {
                    const baseName = fileName.split(/[\\/]/).pop().replace(/\.circ$/, '');
                    const fileLid = app.circuits.addLibrary(baseName);
                    await this.#importFile(fileLid, fileProject, `The library <b><code>${baseName}</code></b>`);
                    libs.push(fileLid);
                }
            } else {
                libs.push(null);
    ...
    #createCircuit(lid, rawCircuit) {
        ...
        const circuit = new Circuit(this.#app, rawCircuit.name, null, [], {}, { parity: "none" }, lid);
    ...
    #convertCircuitComponents(lid, libs, rawCircuit) {
        ...
        for (const rawComp of rawCircuit.comp ?? []) {
            ...
            if (!isLogiSimBuiltIn && layoutId !== null && this.#layouts[layoutId].uid) {
                ...
                const item = new Switch(this.#app, x, y, rotation(rawComp.facing ?? 'east') + 3, 'toggle', rawComp.value === '0x1' ? 1 : 0);
